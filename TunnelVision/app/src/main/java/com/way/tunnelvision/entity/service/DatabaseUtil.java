...
public class DatabaseUtil {
    ...
    public static void initDataBase(Context context) {
        ...
        if(currentVersion < Constants.SCHEMA_VERSION_NEW) {
            helper.onUpgrade(db, Constants.SCHEMA_VERSION, Constants.SCHEMA_VERSION_NEW);
        }
    ...
    public static void addToCollection(Context context, NewsDetailModel newsDetailModel) {
        Log.d(TAG, "addToCollection debug, start");
        DaoMaster.DevOpenHelper helper = new DaoMaster.DevOpenHelper(context, Constants.DATABASE_NAME, null);
        SQLiteDatabase db = helper.getWritableDatabase();
        DaoMaster daoMaster = new DaoMaster(db);
        DaoSession daoSession = daoMaster.newSession();
        NewsDetailDao newsDetailDao = daoSession.getNewsDetailDao();
        String selectionColumn = NewsDetailDao.Properties.NEWSDETAILDOCID.columnName;
        String selection = selectionColumn + " = '"  + newsDetailModel.getDocid() + "'";
        Cursor cursor = db.query(newsDetailDao.getTablename(), newsDetailDao.getAllColumns(), selection, null, null, null, null);
        try {
            if (null != cursor) {
                Log.d(TAG, "addToCollection debug, Cursor Row Count = " + cursor.getCount());
            }
            if (null == cursor || 0 == cursor.getCount()) {
                newsDetailDao.queryBuilder().buildDelete().setParameter(1, "");
                newsDetailDao.insert(newsDetailModel);
                Log.d(TAG, "addToCollection debug, Insert Data Succeed");
            }
        } catch (Exception e) {
            Log.e(TAG, "addToCollection error", e);
        } finally {
            if (null != cursor) {
                cursor.close();
            }
        ...
        Log.d(TAG, "addToCollection debug, end");
    }
}
