    ...
    public static RSSFeed parseXMLWithJSoup(String xmlData) {
        ...
        try {
            ...
            if (null != doc) {
                rssFeed = new RSSFeed();
                String rssTitleText = "";
                if(!doc.select(Constants.RSS_CHANNEL_TITLE).isEmpty()) {
                    rssTitleText = doc.select(Constants.RSS_CHANNEL_TITLE).first().text();
                }
                rssFeed.setTitle(rssTitleText);
                Log.d(TAG, "parseXMLWithJSoup debug, Channel Title = " + rssTitleText);
                String rssDescriptionText = "";
                if(!doc.select(Constants.RSS_CHANNEL_DESCRIPTION).isEmpty()) {
                    rssDescriptionText = doc.select(Constants.RSS_CHANNEL_DESCRIPTION).first().text();
                }
                rssFeed.setDescription(rssDescriptionText);
                Log.d(TAG, "parseXMLWithJSoup debug, Channel Description = " + rssDescriptionText);
                String rssLinkText = "";
                if(!doc.select(Constants.RSS_CHANNEL_LINK).isEmpty()) {
                    rssLinkText = doc.select(Constants.RSS_CHANNEL_LINK).first().text();
                }
                rssFeed.setLink(rssLinkText);
                Log.d(TAG, "parseXMLWithJSoup debug, Channel Link = " + rssLinkText);
                String rssGeneratorText = "";
                if(!doc.select(Constants.RSS_CHANNEL_GENERATOR).isEmpty()) {
                    rssGeneratorText = doc.select(Constants.RSS_CHANNEL_GENERATOR).first().text();
                }
                ...
                if(!doc.select(Constants.RSS_CHANNEL_PUBDATE).isEmpty()) {
                    rssPubDateText = doc.select(Constants.RSS_CHANNEL_PUBDATE).first().text();
                }
                rssFeed.setPubDate(rssPubDateText);
                Log.d(TAG, "parseXMLWithJSoup debug, Channel PubDate = " + rssPubDateText);
                String rssLanguageText = "";
                if(!doc.select(Constants.RSS_CHANNEL_LANGUAGE).isEmpty()) {
                    rssLanguageText = doc.select(Constants.RSS_CHANNEL_LANGUAGE).first().text();
                }
                rssFeed.setLanguage(rssLanguageText);
                Log.d(TAG, "parseXMLWithJSoup debug, Channel Language = " + rssLanguageText);

                Elements items = doc.select(Constants.RSS_ITEM);
                if (null != items && items.size() > 0) {
                    ...
                    for(Element item : items){
                        ...
                        if (!item.select(Constants.RSS_ITEM_TITLE).isEmpty()) {
                            itemTitleText = item.select(Constants.RSS_ITEM_TITLE).first().text();
                        }
                        rssFeedItem.setTitle(itemTitleText);
                        String itemLinkText = "";
                        if (!item.select(Constants.RSS_ITEM_LINK).isEmpty()) {
                            itemLinkText = item.select(Constants.RSS_ITEM_LINK).first().text();
                        }
                        rssFeedItem.setLink(itemLinkText);
                        String itemDescriptionText = "";
                        if (!item.select(Constants.RSS_ITEM_DESCRIPTION).isEmpty()) {
                            itemDescriptionText = item.select(Constants.RSS_ITEM_DESCRIPTION).first().text();
                        }
                        rssFeedItem.setDescription(itemDescriptionText);
                        String itemPubDateText = "";
                        if (!item.select(Constants.RSS_ITEM_PUBDATE).isEmpty()) {
                            itemPubDateText = item.select(Constants.RSS_ITEM_PUBDATE).first().text();
                        }
                        rssFeedItem.setPubDate(itemPubDateText);
                        String itemSourceText = "";
                        if (!item.select(Constants.RSS_ITEM_SOURCE).isEmpty()) {
                            itemSourceText = item.select(Constants.RSS_ITEM_SOURCE).first().text();
                        }
                        rssFeedItem.setSource(itemSourceText);
                        String itemCategoryText = "";
                        if (!item.select(Constants.RSS_ITEM_CATEGORY).isEmpty()) {
                            itemCategoryText = item.select(Constants.RSS_ITEM_CATEGORY).first().text();
                        }
                        rssFeedItem.setCategory(itemCategoryText);
                        String itemGuidText = "";
                        if (!item.select(Constants.RSS_ITEM_GUID).isEmpty()) {
                            itemGuidText = item.select(Constants.RSS_ITEM_GUID).first().text();
                        }
                        rssFeedItem.setGuid(itemGuidText);
                        String itemAuthorText = "";
                        if (!item.select(Constants.RSS_ITEM_AUTHOR).isEmpty()) {
                            itemAuthorText = item.select(Constants.RSS_ITEM_AUTHOR).first().text();
                        }
                        rssFeedItem.setAuthor(itemAuthorText);
                        String itemLink_3gText = "";
                        if (!item.select(Constants.RSS_ITEM_LINK_3G).isEmpty()) {
                            itemLink_3gText = item.select(Constants.RSS_ITEM_LINK_3G).first().text();
                        }
